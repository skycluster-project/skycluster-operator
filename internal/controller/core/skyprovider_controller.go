...
func (r *SkyProviderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// skyProviderDesc := &corev1.ObjectReference{
	// 	Name:       req.Name,
	// 	Namespace:  req.Namespace,
	// 	Kind:       "SkyProvider",
	// 	APIVersion: corev1alpha1.SKYCLUSTER_COREGROUP + "/" + corev1alpha1.SKYCLUSTER_VERSION,
	// }
	...
	// 		corev1alpha1.SKYCLUSTER_PROVIDERNAME_LABEL:   skyProvider.Spec.ProviderRef.ProviderName,
	// 		corev1alpha1.SKYCLUSTER_PROVIDERREGION_LABEL: skyProvider.Spec.ProviderRef.ProviderRegion,
	...
	// 	corev1alpha1.SKYCLUSTER_PROJECTID_LABEL: skyProvider.Labels[corev1alpha1.SKYCLUSTER_PROJECTID_LABEL],
	// }
	...
	// 	skyProvider.Labels[corev1alpha1.SKYCLUSTER_PROVIDERTYPE_LABEL] = providerType
	// 	modified = true
	// }
	...
	// 		Kind:       depObj.GetKind(),
	// 		APIVersion: depObj.GetAPIVersion(),
	// 	}
...
func (r *SkyProviderReconciler) NewSkyProviderObject(ctx context.Context, skyProvider corev1alpha1.SkyProvider) (*unstructured.Unstructured, error) {
	...
	vpcCidr, _, _, err := getSubnetCidr(r.Client, skyProvider)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get IP CIDR parts")
	}
	// I detect a dependency here. Let's break it down.
	// The SkyProvider associates with a provider (aws) region (us-east-1) and zone (us-east-1a)
	// When a new request comes in, a VPC is created for that region given the CIDR range.
	// Then for any zones, falls under that region, a subnet is created with the CIDR range.
	// So, here is the dependency chart:
	// SkyProvidr 1 (aws, region1, zone1) -> VPC (region1), Subnet (zone1)
	// SkyProvider 2 (aws, region1, zone2) -> VPC (region1), Subnet (zone2)
	// When a new SkyProvider object is created, we increment the subnet index.
	// This is because we assign the subnet CIDR to the region, and for each
	// new zone, we increment the subnet index.
	// cidrParts := strings.Split(vpcCidr, "/")
