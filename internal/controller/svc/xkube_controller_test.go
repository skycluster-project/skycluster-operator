...
var _ = Describe("XKube Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind XKube")
			err := k8sClient.Get(ctx, typeNamespacedName, xkube)
			if err != nil && errors.IsNotFound(err) {
				resource := &svcv1alpha1.XKube{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance XKube")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &XKubeReconciler{
				Client: k8sClient,
